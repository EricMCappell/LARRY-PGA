...
async function getDashboard() {
  ...
  // Player photos, keyed by ESPN id, for the roster and leaderboard tables.
  const headshots = {};
  Object.values(money).forEach((bySeason) => {
    Object.values(bySeason || {}).forEach((row) => {
      if (row?.id && row.headshot) headshots[row.id] = row.headshot;
    });
  });
  (snapshot?.leaderboard || []).forEach((p) => {
    if (p.id && p.headshot) headshots[p.id] = p.headshot;
  });
 ...
 *
 * Note the explicit letter swaps. Unicode decomposition handles é → e, but not
 * letters that aren't an accented Latin base: ø, æ, å and friends would
 * otherwise be stripped entirely, so "Højgaard" became "hjgaard" and matched
 * nothing.
 */
const LETTER_SWAPS = [
  [/ø/g, 'o'], [/œ/g, 'oe'], [/æ/g, 'ae'], [/å/g, 'a'],
  [/ð/g, 'd'], [/đ/g, 'd'], [/þ/g, 'th'], [/ł/g, 'l'], [/ß/g, 'ss'],
];
...
function nameKey(name) {
  let out = String(name || '')
    ...
    .replace(/[̀-ͯ]/g, '')
    .toLowerCase();
  LETTER_SWAPS.forEach(([from, to]) => { out = out.replace(from, to); });
  return out
...
async function refresh({ captureBaseline = null } = {}) {
  ...
  // Compare dates in Eastern time, not UTC — otherwise "the pool starts Sept 17"
  // silently means 8pm Eastern on the 16th.
  const todayEastern = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/New_York' }).format(new Date());
  const poolStarted = todayEastern >= settings.poolStart;
