...
/**
 * ESPN refuses requests from datacenter IP ranges with a 403, which means a
 * cloud host (Vercel, most CI runners) may not be able to reach it even though
 * a laptop can. Set ESPN_PROXY to the URL of something that can — see README —
 * and every request is routed through it as `?url=<encoded original>`.
 */
...
async function fetchJson(url, { timeout = 15000, attempts = 2 } = {}) {
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      if (res.status === 403) {
        throw new Error(
          `ESPN responded 403 for ${url} — it blocks datacenter IPs. Run the refresh from a machine it allows, or set ESPN_PROXY.`
        );
      }
      ...
      lastError = err;
      if (attempt < attempts && !String(err.message).includes('403')) {
        // eslint-disable-next-line no-await-in-loop
